...
namespace motioncam {
    ...
    public:
        ...
        // Read the raw on-disk bytes of a frame WITHOUT decompressing the bayer.
        // Used by the trim writer to do bit-perfect copies. outCompressedBuffer
        // gets the raw BUFFER chunk; outFrameMetadataJson gets the unparsed
        // frame metadata JSON (so callers can re-emit it byte-for-byte).
        void loadFrameRaw(Timestamp timestamp,
                          std::vector<uint8_t>& outCompressedBuffer,
                          std::string& outFrameMetadataJson);
